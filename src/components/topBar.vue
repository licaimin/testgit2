<template>
	<div class="topBar">
    	...
    	<div class="greyCtn" v-if='isLgShow' id="greyCtn">
    		<login-bar v-on:listenClose='recieveCancel' v-bind:tab='tab'></login-bar>
    	</div>
	</div>
</template>
<script>
	...
	export default{
		...
		methods:{
	     ...
	     //    },
	        ...
	        connection() {
	            let socket = new SockJS('http://119.29.176.47:9001/notification');
	            let token=this.getCookie('token')
		...
		mounted(){
	        let that = this;
	        let id=(JSON.parse(this.getCookie('userData'))).id
	        this.$axios({
	          method: 'get',
